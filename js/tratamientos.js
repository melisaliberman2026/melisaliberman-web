...
// =============================================================
import "./main.js";
import { TRATAMIENTOS } from "./datos.js?v=2";
import { escapar } from "./util.js?v=2";
...
if (caja) {
  caja.innerHTML = TRATAMIENTOS.map(
    ...
    <details class="acor" id="${escapar(t.id)}"${i === 0 ? " open" : ""}>
      <summary>
        <span class="acor__ico"><svg><use href="assets/iconos.svg?v=2#ico-${escapar(t.icono)}"></use></svg></span>
      ...
      <div class="acor__cuerpo">
        ...
        <ul>
          ${t.puntos
            .map(
              (p) =>
                `<li><svg><use href="assets/iconos.svg?v=2#ico-check-simple"></use></svg><span>${escapar(p)}</span></li>`
            )
            .join("")}
        </ul>
      </div>
